...
const MapPage = (() => {
  ...
  function renderPins() {
    ...
    if (!pinLayer) return;
    const points = CONFIG.COLLECTION_POINTS || [];
    pinLayer.innerHTML = points.map((p) => `
      <button class="map-pin" style="left:${p.x || 50}%; top:${p.y || 50}%" type="button" title="${p.name || ''}">${PIN_ICON}</button>
    `).join('');
  }
  ...
  function renderList() {
    ...
    list.innerHTML = points.map((p) => `
      <div class="action-item is-note"><span class="ico">${PIN_ICON}</span>${p.name || 'Collection Point'}</div>
    `).join('');
  }
